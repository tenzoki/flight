...
Get the timestamp from `date +"${FLIGHT_FILE_PREFIX:-%Y-%m-%d_%H-%M}"` (env var `FLIGHT_FILE_PREFIX` overrides the default; default renders as `YYYY-MM-DD_HH-MM`). If the **Open tasks** section starts with the placeholder text `(No open tasks yet...)`, remove that line first.
...
Write the memo to `./flight-workbench/memos/<prefix>-<slug>.md` (where `<prefix>` comes from `date +"${FLIGHT_FILE_PREFIX:-%Y-%m-%d_%H-%M}"`):
